...
- **v1.6（2026-07-05）**: `gadget.ai.complete` に任意 `tier`（`'fast'|'smart'`）を追加。モデル名は
  指定せず、プラットフォームが `tier × ユーザーのプロバイダ` を具体モデルに解決する（提供元非依存・
  後方互換）。省略時は従来どおりユーザー設定のモデル。streaming と §9 のツール開放は今後の段階導入
