...
| 部屋（へや） | 各ユーザーの主画面（ログイン後の起点）。募集コピー「あなたの部屋を、長屋に」の「部屋」 | dashboard / home |
| 棚（たな） | 部屋の中で道具（ガジェット）を並べる領域 | ダッシュボード内のガジェット一覧 |
| 道具（どうぐ） | ガジェット | gadget |
| 道具市（どうぐいち） | カタログ | catalog |
| 工房（こうぼう） | 職人の開発画面・入口 | 開発者向け導線 |
| 大家（おおや） | 管理者 | admin |
| 職人（しょくにん） | 道具をつくる人（自己申告の呼称） | user |
| 店子（たなこ） | 道具をつかう人（自己申告の呼称） | user |
