...
| 8 | 独自SMTPの設定（メンバー招待前に必須） | Supabase 内蔵メールは1時間に数通の制限があり、マジックリンク/登録確認メールが「email rate limit exceeded」になる。**建立会でメンバーが一斉登録するとすぐ上限に当たる**ため、Resend 等の独自SMTPを Supabase（Authentication → SMTP settings）に設定する。あわせて送信元ドメイン（将来 admin@nagaya-base.app 等）の整備。回避策: パスワードログインとゲスト（匿名）はメールを送らない | 建立会の前 |
