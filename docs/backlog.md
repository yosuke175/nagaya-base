...
| 7 | gadget.ai の複数プロバイダ対応 | 現在 `functions/api/ai.ts` は Anthropic（Claude）のみ。案内所FAQで「お好みのAI（Claude/OpenAI/Google）を選べる」と案内しているため、内蔵AI機能も OpenAI / Google 等を選べるようにする（AI設定にプロバイダ選択を追加、/api/ai を分岐）。BYOK外部サービス経由なら現状でも他社は使える | メンバー要望を見て |
