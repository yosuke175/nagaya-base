...
## 2026-07-06（直前の修正が招いた重大な回帰＝スクロール量ぶん縦に大きくズレるバグを修正）

向井「もう全然ダメ。ガジェットを掴んだら250pxぐらい下に移動する（案内AIは平気）」。直前のDOM実測化
（`measureRect`）が原因の**新規の回帰**だった。

- **原因**: `getBoundingClientRect()` は常に**ビューポート基準**（スクロール量を含む）の座標を返す。
  一方 `position:absolute` の `top`/`left` は**containing block（`position:relative`の親）基準**の座標
  （ページがスクロールしても、その親要素の中での相対位置は変わらない）。この2つの座標系は、
  containing block 自身がビューポート内のどこにあるか＝スクロール量ぶんだけ食い違う。
  前回の `measureRect` は補正なしで `getBoundingClientRect()` の値をそのまま使っていたため、
  ページが約250pxスクロールしていると、その分だけ縦にズレた値を`top`として設定してしまっていた。
  **案内AI（`position:fixed`）は影響を受けない**: fixed要素のtop/leftは元々ビューポート基準なので、
  `getBoundingClientRect()`の値とそのまま一致し、補正が要らない。これが「案内AIは平気」の理由。
- **修正**: `measureRect` で、要素とcontaining block両方の `getBoundingClientRect()` を取り、
  **containing blockの位置を差し引いて** containing-block相対座標に変換してから使うようにした
  （`toCenterOffset`も同様、差し引き後の座標系で計算するよう整合）。
- 検証: ブラウザ上で「ページを250pxスクロールした状態」を再現し、`measureRect`が正しく
  スクロール量に関係なく一定の値（containing-block相対のy=42）を返すこと、往復（実測→cx変換→
  calc(50%+cx)で再描画）がスクロールありでもピクセル単位で完全一致することを確認。
  build＋78テスト緑。

## 2026-07-06（掴む/離す瞬間のズレを恒久対策＝ビューポート幅からの計算をやめてDOM実測に）

向井「スクロールバーは出てないのに、ガジェットを掴むと文字1つ分右にズレ、離すと文字1つ分左に戻る
（案内AIは平気）」。前回のスクロールバー対策後もなお再現とのことで、根本的に作り直した。

- **方針転換**: 「ビューポート幅(document.documentElement.clientWidth)から絶対座標を逆算する」
  （`rectFromCenter`/`centerFromRect` 経由）のをやめ、**実際に描画されているDOMを
  `getBoundingClientRect()` で直接測る**方式に置き換えた。100vw・パディングの入れ子・
  スクロールバー等、CSSの containing block 解決に影響しうる要因がいくら重なっていても、
  「今まさに画面に出ている値」を直接読むので原理的にズレが起こり得ない。
- **FloatingWindow**: ドラッグ開始時は `measureRect(containerRef.current)` で今の絶対座標を
  そのまま `orig`/`dragLocal` の初期値にする（＝掴んだ瞬間に見た目が変わらない）。ドラッグ終了時は
  `toCenterOffset()`で **containing block（`el.offsetParent` = 棚の`.relative`）を実測**して
  cxに変換する（＝離した瞬間、次に`calc(50%+cx)`で描画される位置が実測結果と一致する）。
- **GuideAssistant**: 同じ考え方を適用。`position:fixed`のため`offsetParent`が無く、
  containing block は「初期包含ブロック」＝`document.documentElement`を実測して使う。
- 検証: build＋78テスト緑。ブラウザ上で「実測→cx変換→calc(50%+cx)で再描画」のラウンドトリップが
  ピクセル単位で完全一致する（769.5=769.5）ことを実測確認。ビューポート幅ベースの計算を排除した
  ため、100vw/パディング/スクロールバーいずれの要因からも原理的に独立し、再発の可能性を潰した。

## 2026-07-06（マウスを離した瞬間のズレの真因＝スクロールバーの出現/消滅。永続的に幅を確保して解消）

向井「スクロールバーは出てないし、案内AIは平気なのにガジェットだけ、動かして離した瞬間ブルっとズレる」。
前回の clientWidth 統一だけでは直っておらず、再度実機で構造を検証して真因を特定:

- **`deskHeight`（棚の`.relative`コンテナの高さ）は、ドラッグ中ずっと"確定済み"のレイアウト
  （`rawLayouts`）から計算される。ドラッグ中に見えている大きさ・位置(`dragLocal`)はローカルな
  ワーキングコピーで、`deskHeight`には反映されない。** そのため、窓を大きく／下に動かす操作の
  「最中」はページの高さが変わらずスクロールバーの状態も変化しないが、**マウスを離してcommitされた
  瞬間**に初めて`deskHeight`が実際の新サイズで再計算され、ページの合計の高さがビューポートの高さを
  またいで**縦スクロールバーが新たに出現/消滅**することがある。スクロールバーの出現は
  `document.documentElement.clientWidth` を（スクロールバー幅ぶん）変化させるため、**すでに
  committされた cx（画面中央からのオフセット）が、その瞬間から急に別の基準幅で解決されてしまい**、
  ページ上の**全ガジェット**が一瞬だけズレて見える。
- **案内AI（`position:fixed`）は対象外**: fixed要素は通常フロー・スクロール高さに寄与しないため、
  移動・リサイズしてもページの高さ自体が変わらず、スクロールバーの出現/消滅を引き起こさない。
  これが「案内AIは平気・ガジェットだけズレる」の正体だった。
- **修正**: `html { overflow-y: scroll; scrollbar-gutter: stable; }` を追加。縦スクロールバー用の
  余白を常に確保し、コンテンツの高さに関わらず `clientWidth` を恒常的に一定にする。これにより、
  ガジェットの移動・リサイズでページの高さが変わっても、スクロールバーの出現/消滅そのものが
  起こらなくなり、`cx`の解決基準が変化する余地が無くなる。
- 検証: ブラウザ上で、コンテンツ高さを1200→4200pxに変化させても `clientWidth` が1185pxのまま
  一定であることを実測確認（修正前は1200→1185に変化していた）。build＋78テスト緑、起動確認OK。
