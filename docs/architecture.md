...
**状態**: P2/P3 実装済み（2026-07-05・gadget-spec v1.6 §9）。P4（PC個別会話・段3記憶統合）は後続
...
## ADR-012: ガジェット開発＝クラウド編集＋プラットフォーム内プレビュー（ローカルclone/dev-server の撤廃）

**状態**: 提案（2026-07-06・向井が方向を承認）。**Phase 0/1/2 第一版を実装（2026-07-06）**。Phase 2 は**方式A＋`sandbox allow-scripts` CSP**を採用（下記）。実機E2Eは要検証。ADR は追記のみ＝ADR-001/002/003/004/006 は不変で、必要な補足のみ持つ。

**背景**: 現行の「fork → ローカルclone → npm install → ローカル開発サーバ」は、非エンジニアの入居者に摩擦が大きく「腑に落ちない」との指摘（2026-07-06）。調査の結果、向井の提案（リポジトリ内・ユーザー別ディレクトリで開発／プラットフォームはソース read-only／公開＝DBフラグ）は**大半が現行アーキで既に成立**していることが判明:
- web はガジェットを静的配信するのみでソースに書けない（ADR-001/004）。
- monorepo が既に「1道具＝1ディレクトリ（`gadgets/<id>/`）」の共有作業場（ADR-002）。
- 公開の実体は `gadgets.status`（RLS: owner/admin のみ更新）で、工房から web 完結で切替可能（ADR-003）。
- 実行時のサンドボックス境界は完成済み（ADR-001）。
現状とのズレは実質2点のみ: (a) 開発がローカルclone必須、(b) 作りかけの試用がローカル開発サーバ限定。

**決定**:
1. 開発は**各自の fork** に対して**クラウド編集**（GitHub Codespaces もしくは各コーディングツール〔Claude Code / Codex / Antigravity〕で fork を開く）で行う。**ローカル clone / 開発サーバを必須にしない。**
2. **git 書き込みモデルは fork + PR を維持**（ADR-006 の CLA・著作権処理が PR で機能するため。同一リポのブランチ運用＝全員へのコラボレーター権限付与は採らない）。
3. **作りかけの試用は「自分の部屋」でプレビュー**できるようにする（＝本ADRの唯一の新規実装点）。未マージの `gadgets/<id>/` を、**プラットフォームのオリジン配下・本人セッション限定・ADR-001 のサンドボックス維持**で iframe 表示する。
4. **不変条件: 「道具市への公開」は必ず PR→CI→マージ→`status=published` を通す。** プレビューは公開ではない。プレビュー経路が審査/CLA を迂回する裏口になってはならない（ADR-006）。

**プレビュー方式（Phase 2）— 採用: 方式A（gitベース）**:
- 実装: Pages Function `functions/preview/[[path]].ts` が、本人 fork（リポジトリ名は本家を継ぐ）の `gadgets/<id>/` を、プラットフォーム自身のオリジン配下 `/preview/<owner>/<branch>/<gadgetId>/...` で中継配信。GadgetFrame に `basePath` を足し、`/preview/…` を渡して本番同等のホスト（SDK/保存/AI/連携）で試運転する。工房に GitHubユーザー/ブランチ/道具ID を入れる導線（`GadgetPreview`）。
- **安全性の肝＝`sandbox allow-scripts` を CSP レスポンスヘッダで付与**: プレビューHTML文書を（iframe 内でも、URL 直開きの最上位でも）不透明オリジンのサンドボックスに強制する。これで未マージ＝未レビューの fork コードがプラットフォームのセッション/localStorage（Supabase トークン等）に到達できない。別オリジンや署名トークンを新設せずに「同一オリジン配信の危険」を封じる。connect-src は道具の manifest 宣言 baseUrls のみ。GET 限定・本家リポ名のみ・`gadgets/<id>/` 配下のみ（open-proxy / traversal 防止）。
- 検討したが不採用: 方式B（アップロード保管＝git非経由で CLA/履歴と切れる・保管掃除の運用増）／別オリジン Pages プロジェクト（インフラ増）／署名トークン（相対アセットにクエリが継がれず実装が煩雑）。方式A＋sandbox-CSP が最小で安全。

**セキュリティ（両方式共通・ADR-001 踏襲）**: iframe は allow-scripts のみ／allow-same-origin 付与禁止／connect-src はマニフェスト宣言 baseUrls のみ／Supabase トークンは iframe に一切渡さない／プレビューは owner(auth.uid) セッション限定（他人の作りかけを踏めない）／別オリジン配信は禁止（必ずプラットフォームのオリジン配下にプロキシ/仮想配信）。

**段階**:
- **Phase 0（安全・すぐ）**: docs/impl のズレ解消（README/gadget-spec の `npm run dev:gadget` を実装〔`npm run dev`〕に、未実装の「公開CI」記述を実態に）。
- **Phase 1**: オンボーディング導線を「fork→クラウド編集」に。ウィザード/工房から「ローカル clone / 開発サーバ必須」を外す。公開は従来 PR。
- **Phase 2**: 部屋プレビュー基盤（方式A/B 確定後に実装）。
- **Phase 3**: 公開ゲートの CI 実装（マニフェスト検証・宣言外API静的検査・ビルド）＋ `status=published` 連動。

**理由**: 提案の大半は既に成立しており、真の新規実装は「未マージコードの本人限定プレビュー」1点に集約される。段階導入で、まず摩擦（clone/dev-server）を消し、次にプレビュー、最後に公開 CI を固める。
