...
**状態**: 提案（2026-07-06・向井が方向を承認）。段階導入。Phase 2 のプレビュー方式は要確定（下記）。ADR は追記のみ＝ADR-001/002/003/004/006 は不変で、必要な補足のみ持つ。
...
**プレビュー方式（Phase 2・要確定）**:
- 方式A（推奨）: 本人の fork/ブランチの `gadgets/<id>/` を、プラットフォームの軽量 Worker が**自オリジン配下にプロキシ**して iframe 配信。利点: すべて git に残り CLA/所有権/履歴と整合、別ストレージのライフサイクル不要。難点: Worker 新規、未マージ＝未CI コードを本番オリジンで動かすため本人限定・サンドボックス厳守が必須。
- 方式B（代替）: 工房からソース/ビルドを**アップロード**→Supabase Storage / Workers KV に「本人×プレビュー」名前空間で保管→自オリジン配信。利点: CSP/SDK import がそのまま・push 前でも見られる。難点: git 非経由なので「プレビュー可・公開は必ず PR」の分離規約が必須、保管の TTL/容量/掃除運用、ADR-004 の「コード=Cloudflare」からの逸脱。
