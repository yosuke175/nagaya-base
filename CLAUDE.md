...
ガジェットの仕様契約は `docs/gadget-spec.md`、見た目の憲法は `docs/frontend-design.md` を参照。
**これら文書と矛盾するコードを書かないこと。矛盾に気づいたらコードではなく人間に報告すること。**

## 世界観の語彙（語彙帳）

UI表示文字列はこの語彙で統一する（内部の識別子・role値は英語のまま）。
正典は `docs/frontend-design.md`。名称・語彙はハードコードせず設定/定数から参照する。

| 世界観の語 | 意味 | 内部の対応 |
|---|---|---|
| 長屋 | プラットフォーム全体 | platform |
| 部屋 | 各ユーザーの主画面（ログイン後の起点）。募集コピー「あなたの部屋を、長屋に」の「部屋」 | dashboard / home |
| 棚 | 部屋の中で道具を並べる領域（ガジェット一覧） | ダッシュボード内のガジェット一覧 |
| 道具 | ガジェット | gadget |
| 道具市 | カタログ | catalog |
| 工房 | 職人の開発画面・入口 | 開発者向け導線 |
| 大家 | 管理者 | admin |
| 職人 | 道具をつくる人（自己申告） | user |
| 店子 | 道具をつかう人（自己申告） | user |
| 軒先 | ゲスト | guest |
| 回覧板 / 速報！ / 長屋暦 / 案内所 / 長屋の歩み | 情報系レイヤー | announcements / activity_feed / events / help / progress |
...
| ロール | できること |
|---|---|
| admin（大家） | プラットフォーム管理、ロール付与、ガジェットの緊急停止 |
| user（店子/職人） | ガジェットのインストール・利用・設定 ＋ **登録・更新・公開** |
| guest（軒先） | 公開カタログの閲覧のみ（インストール不可） |

- **入居者のロールは `user` 1つ**（2026-07-04、旧 `developer` を統合）。「職人／店子」は
  自己申告の呼び方でロールの差はない。enum の `developer` は残すが今後付与しない
- guest→user はメール登録で自動付与（サインアップ時のトリガー）、admin への昇格は admin のみ
