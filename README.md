...
- 認証（FR-01 一部）: マジックリンクログイン・ログアウト・プロフィール/ロール表示。
  `.env` に Supabase 未設定の場合はログインなしのローカル開発モードで起動。
  guest ロールはカタログ閲覧のみ（インストール不可。RLSでも同じ制約を強制）
- 未実装: Google ログイン・データ保存の Supabase 置き換え（storage / installations /
  承認は現在ブラウザ内モック）・ガジェット登録フロー（Phase 2）
...
| 環境変数（Plaintext） | `NODE_VERSION=22` / `VITE_APP_NAME=NAGAYA-BASE` / `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` / `SUPABASE_URL`（VITE_ と同じ値） |
| 環境変数（**Secret**） | `SUPABASE_SERVICE_ROLE_KEY`（Supabase の service_role） / `CREDENTIALS_ENCRYPTION_KEY`（32バイトのランダム値の base64） |

Secret 2つは `functions/api/credentials.ts`（クレデンシャルのサーバー側暗号化保管、ADR-005）
専用。**クライアント側の変数（VITE_）に service_role を入れてはならない**。
`CREDENTIALS_ENCRYPTION_KEY` の生成例（PowerShell）:

```powershell
$b=[byte[]]::new(32);[Security.Cryptography.RandomNumberGenerator]::Fill($b);[Convert]::ToBase64String($b)
```

※ Pages Functions はリポジトリ直下の `functions/` に置く（Pages が Root directory
基準で解決するため。CLAUDE.md の構成図にある `platform/functions/` とは異なる）。
