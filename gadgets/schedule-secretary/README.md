...
## AIチャット（v0.2で導入、v0.5でAIに相談と統合）

- 「AIに相談」（一発提案フォーム）と「AIチャット」は同じAI相談機能が重複して
  ユーザーに分かりにくかったため、v0.5 でチャット1本に統合した。会話の中で
  「空いてる時間ある？」と相談することも、予定の作成・移動・削除を頼むこともできる
- AI は `gadget.ai.complete`（permissions: `"ai"`）経由。**API キーはガジェットに
  渡されず**、プラットフォームの「AI設定」（工房）に登録したユーザー自身のキーが使われます
- 個人ルールは自由記述で `gadget.storage` に保存（例: 平日午前は予定を入れない）。
  設定ページで編集し、チャットの提案時に厳守させる

## 設定ページ（v0.5）

- 「⚙ 設定」ボタンで、カレンダー設定・個人ルール・AIチャットのオン/オフをまとめた
  設定ページに切り替えられる（本体の予定表を広く使えるようにするため分離）

## 複数カレンダー対応（v0.4、色対応はv0.5）
...
- カレンダーの色は Google カレンダー本家の色（`Calendar.getColor()` の colorId）を
  取得し、Calendar API の固定パレット（1〜24）で16進色に変換して表示に反映する。
  colorId が取れない/未知の場合のみ、calendarId から機械的に色を割り当てる
...
- チャット窓は設定ページの「このガジェット内のAIチャットを使う」トグル（または
  パネル右上の「隠す」）で非表示にでき、非表示中でも `gadget.ai.onToolInvoke` は常に
...
- v0.5: 設定ページ分離・本家カレンダー色対応・AIに相談とAIチャットの統合（実装済み）
