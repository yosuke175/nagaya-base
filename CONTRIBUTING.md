...
早見表で「△」にあたること（他人のコードを有償の製品に組み込む等、
プロジェクト成果物の商用利用で判断が必要なもの）は、**事前に**稟議を通してください。

1. Issue を「商用利用の相談」として立てる（**何を・どこで・どのように**使うかを記載）
2. プロジェクト管理者（向井）と協会側の承認を得る（= ハンコ2つ）
3. 承認の内容と範囲は、その Issue 上に記録として残す

判断に迷う場合も、まず Issue で相談してください。相談は歓迎されます。
